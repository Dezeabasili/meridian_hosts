...
const Chats = () => {
  ...
  useEffect(() => {
    const getAllChats = async () => {
      try {
        setLoading(true);
        const res1 = await axiosWithInterceptors.get("api/v1/chats");
        // console.log("chats: ", res1.data);
        setChats(res1.data);

        const res2 = await axiosWithInterceptors.get("api/v1/users");
  ...
  const downloadMessages = async (chat) => {
    try {
      ...
      const res = await axiosWithInterceptors.get(`api/v1/messages/${chat._id}`);
